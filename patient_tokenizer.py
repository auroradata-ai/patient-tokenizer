...
def process_csv(input_csv: str, output_csv: str, salt: str):
    """
    Reads a CSV file, tokenizes the 'name' column, and writes the tokens to a new CSV.
    """
    # Handle "env:" prefix for salt
    if salt.startswith("env:"):
        salt = os.environ.get(salt[4:], "")
        if not salt:
...
def _cli():
    ...
    p = argparse.ArgumentParser(
        ...
        description=textwrap.dedent(
            ...
            Process a CSV file
                $ python patient_tokenizer.py process_csv \\
                     --input_csv input.csv --output_csv output.csv \\
                     --salt env:PT_SALT
            """
        ),
    )
    ...
    elif args.cmd == "process_csv":
        process_csv(args.input_csv, args.output_csv, args.salt)
